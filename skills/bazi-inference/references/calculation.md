...
边界计算状态限制的是“唯一命盘及全期稳定结论”的声明，不是题卷答案的交付。完成合法分支比较后，按用户给定口径或明确的情景提交最终候选并说明敏感性；若条件仍不足，标为猜测，不伪造精确出生时间，也不把程序状态直接抄成弃答理由。
...
`decision-record.md` 的分支字段记录实际分析过的各段；期望分支清单来自本层计算。v3每段记录完整候选排序或明确并列及理由，不能只保存边界说明；缺清单的旧记录不能宣称完整覆盖。候选事件年互斥时，每个共同时间口径情景仍比较全部年份，具体年内段作为依据，不把不同候选年的任意片段伪装成同一个观察时刻。
